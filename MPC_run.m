...
if expt_param.dofmri
    device(2).product = 'KeyWarrior8 Flex'; % syncbox
...
if expt_param.dofmri
    ...
    Screen('Flip', theWindow);
    waitsec_fromstarttime(data.dat.fmri_start_time, 8);
end
...
% EYE TRACKER in 7T
if expt_param.USE_EYELINK
    data.dat.eyetracker_starttime = GetSecs; % eyetracker timestamp
    
    if ~strcmp(expt_param.run_type, 'resting')
        data.dat.eyetracker_cali_starttime = GetSecs; % eyetracker calibration timestamp

        cal_moviename = fullfile(pwd, '24points_calibration_40sec_1600x1000.mp4');
        [cal_movie, duration] = Screen('OpenMovie', theWindow, cal_moviename);

        % Video Start
        Screen(theWindow, 'FillRect', bgcolor);
        Screen('Flip', theWindow);
        ...
        while (t2-vidtime)<duration
            ...
            tex = Screen('GetMovieImage', theWindow, cal_movie);
            if tex<=0
                break;
            end
        ...
        data.dat.eyetracker_cali_endtime = GetSecs; % eyetracker calibration timestamp
    end
...
%% Wating seconds from fmri started
%waitsec_fromstarttime(data.dat.fmri_start_time, 13);
if ~strcmp(expt_param.run_type, 'resting')
    if expt_param.USE_EYELINK
        waitsec_fromstarttime(data.dat.fmri_start_time, 56); % 7T: 8s+40s(eyetracker cali)+8s = 56s
    end
end
...
elseif strcmp(expt_param.run_type, 'movie_heat') % Movie heat Run
    for Trial_num = 1:expt_param.Trial_nums % movie -> no_movie -> movie -> no_movie ... sequence
        ...
        %if mod(Trial_num,2) == 0 % even number trial
...
%     eyelink_main(edfFile, 'Shutdown');
    data.dat.eyetracker_endtime = GetSecs;
end
...
data.dat.run_dur = data.dat.run_endtime - data.dat.run_starttime;


%% wait 16 secs for multi-echo (all runs)
%if strcmp(expt_param.run_type, 'movie_heat') || strcmp(expt_param.run_type, 'no_movie_heat')

Screen(theWindow, 'FillRect', bgcolor, window_rect);
Screen('Flip', theWindow);
Screen('TextSize', theWindow, fontsize);

waitsec_fromstarttime(data.dat.run_endtime, 16); % for 7T
%WaitSecs(16); 

%% fmri end time
data.dat.fmri_endtime = GetSecs;
data.dat.fmri_dur = data.dat.fmri_endtime - data.dat.fmri_start_time;
